  ...
  return (
    <div className="xl:hidden">
      <button
        ...
        aria-expanded={open}
        className="inline-flex h-10 items-center gap-2 rounded-[2px] px-2 text-[0.78rem] font-semibold uppercase tracking-[0.16em] text-current hover:bg-foreground/[0.06]"
        onClick={() => setOpen((v) => !v)}
        type="button"
      >
      ...
      >
        <nav aria-label="Main navigation (mobile)" className="container py-2">
          <ul className="flex flex-col divide-y divide-border">
            ...
            <li>
              <Link
                className="flex items-center justify-between gap-4 py-3.5 text-[0.78rem] font-semibold uppercase tracking-[0.16em] text-muted-foreground"
                href="/search"
              >
                Search the site
              </Link>
