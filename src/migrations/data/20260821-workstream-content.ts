/**
 * FROZEN. Do not edit this file to change what the site says.
 *
 * This is the data the 20260821 migration backfilled the workstream detail
 * fields with, kept beside the migration that uses it. A migration must keep
 * producing the same result forever, so it cannot import content that anybody
 * might reasonably edit — it used to import this from the seed, which meant
 * every future copy change silently rewrote history for any database being
 * built from scratch.
 *
 * The live copy is in the production CMS. Edit it there.
 *
 * Transcribed from MHG_boundary_positions1.xlsx (compiled Aug 2026).
 *
 ...
 *
 * REVIEW STATUS at the time it was written: the sheet's own annotation
 * (column A) read "Have only included suggestions; respective workstream team
 * to EDIT final." Sign-off happens in the admin, against the live content —
 * not here.
 */
