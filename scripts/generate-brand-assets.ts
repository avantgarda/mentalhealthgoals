...
const writeVariant = async (variant: LogoVariant): Promise<string[]> => {
  ...
  // lockup-email.png is not rasterised here: it is the site's own header
  // lockup, photographed by `pnpm generate:email-lockup` (needs the site running).
...
Generated by \`pnpm generate:brand\` from \`src/brand/marks.ts\` (and \`lockup-email.png\` by
\`pnpm generate:email-lockup\`). **Do not edit these
...
| \`lockup-email.png\` | The website header's lockup, photographed at 2x from the real component and fonts by \`pnpm generate:email-lockup\` (needs the site running) — the header of every email the site sends, where SVG will not render |
